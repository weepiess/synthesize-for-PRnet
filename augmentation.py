...
# from face3d.morphable_model import fit
...
def synthesize(image,label,face_ind,triangles,angle,landmark3d,w=256,h=256):
    ''' synthesize
    Args:
        image: [256,256,3]
        label: [256,256,3]
    ...
    Returns:
        rotated vertices: [nver, 3]
        rotated image: [256,256,3]
        rotated pomap: [256,256,3]
        2d landmark: [n,2]
    '''
    t1 = time.clock()
    turn_angle = angle
    ...
    pos_gt = label
    all_vertices_gt = np.reshape(pos_gt, [w*h, -1])
    #print('vsize: ',all_vertices_gt.shape)
    temp = all_vertices_gt
    vertices_gt = all_vertices_gt[face_ind, :]

    vertices_gt[:,0] = np.minimum(np.maximum(vertices_gt[:,0], 0), w - 1)  # x
    vertices_gt[:,1] = np.minimum(np.maximum(vertices_gt[:,1], 0), h - 1)  # y
    ...
    all_colors_r = np.reshape(ref_texture_r, [w*h, -1])
    text_c_r = all_colors_r[face_ind, :]
    #print('angle: ',[pitch_angle,turn_angle])
    rv = rotate(vertices_gt,turn_angle)
    temp[face_ind,:] = rv
    uv_position_map = np.reshape(temp,[h,w,3])


    pic_gt2 = render_texture(rv.T,text_c_r.T,triangles.T,h,w)
    mark2d = []
    for i in range(len(landmark3d)):
        ...
        mark2d.append([x,y])
    print(time.clock() - t1)
    return pic_gt2,uv_position_map,rv,mark2d
